 ...
 * tracking the state changes on a Cooking Plate.
 *
 * <p><strong>Description</strong></p>
 * 
 * <p>
 * The model receives event from the Cooking Plate component (corresponding to
 * calls to operations on the Cooking Plate in this component), keeps track of
 * the current state of the Cooking Plate in the simulation and then emits the
 * received events again towards another model simulating the electricity
 * consumption of the Cooking Plate given its current operating state (switched
 * on/off, 0 to 7 power mode).
 * </p>
 * <p>
 ...
 * state model will serve as a bridge between the models put in the Cooking Plate
 ...
 *   {@code DecreaseCookingPlate},
 *   {@code IncreaseCookingPlate}</li>
 * <li>Exported events:
 *   {@code SwitchOnCookingPlate},
 *   {@code SwitchOffCookingPlate},
 *   {@code DecreaseCookingPlate},
...
implements CookingPlateOperationI {
	...
	public static final String MIL_URI = CookingPlateStateModel.class.getSimpleName() + "-MIL";

	/** URI for an instance model in MIL real time simulations; works as
	 *  long as only one instance is created.								*/
	public static final String	MIL_RT_URI = CookingPlateStateModel.class.getSimpleName() + "-MIL_RT";

	/** URI for an instance model in SIL simulations; works as long as
	 *  only one instance is created.										*/
	public static final String	SIL_URI = CookingPlateStateModel.class.getSimpleName() + "-SIL";

	/** current state of the Cooking Plate.									*/
	...
	/**
	 * create a Cooking Plate state model instance.
	 * 
	...
	public void increaseMode() {
		// a increaseMode() event can only be executed when the state of the 
		// Cooking Plate model is in the state ON
		if (this.currentState == CookingPlateElectricityModel.CookingPlateState.ON) {
			this.currentMode = (this.currentMode +1) % 8; // TODO A voir %
		}
	}
	...
	public void decreaseMode() {
		// a decreaseMode() event can only be executed when the state of the 
	...
	public void userDefinedExternalTransition(Duration elapsedTime) {
		...
		// and for the Cooking Plate model, there will be exactly one by
		// construction.
